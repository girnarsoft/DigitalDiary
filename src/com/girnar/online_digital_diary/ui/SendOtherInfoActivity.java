...
public class SendOtherInfoActivity extends Activity implements ImportantMethod,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		tracker.trackView("Send the Other Information Digital_Diary");
		getIds();
		addListener();
		...
		SendOtherInfoAdapter adapter = new SendOtherInfoAdapter(
				SendOtherInfoActivity.this, items, listDataChild,true);
	...
	public void getIds() {
		...
		header_text.setText("Send Other Info");
	...
	public void addListener() {
		...
		editTextSearch.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before,
				...
				db.open();
				SharedPreferences preferences = PreferenceManager
						.getDefaultSharedPreferences(SendOtherInfoActivity.this);
				int id = preferences.getInt("id", 0);
				...
				if (cursor != null) {
					...
					for (int i = 0; i < cursor.getCount(); i++) {
						...
						if (checked_ids.contains(item.getId() + "checked")){
							SendOtherInfoActivity.setSendInfoIds(
				...
				SendOtherInfoAdapter adapter = new SendOtherInfoAdapter(
						SendOtherInfoActivity.this, items, listDataChild,false);
				listView.setAdapter(adapter);
				adapter.notifyDataSetChanged();
	...
	public void onClick(View v) {
		...
		if (v == back) {
			...
			for (int i = 0; i < selected_ids_for_send.size(); i++) {
				...
				db.open();
				Cursor cursor = db.getInfoBookmarkMethod(selected_ids_for_send
						.get(i));
				db.close();
				...
				String text = (i + 1) + ". \tTitle := " + title
						+ "\n\tDescription :=" + description + "\n";
				selected_text_for_send += text;
			}
			if(selected_ids_for_send.size()==0){
				Toast.makeText(this, "Please select other informations for send", Toast.LENGTH_LONG).show();	
				}else{
